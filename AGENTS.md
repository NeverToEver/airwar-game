...
## 12. 任务跟踪 / Task Tracking（2026-07-20 阶段归档）

2026-07-17 启动的「高优先级执行任务」历经三天推进（2026-07-17 → 2026-07-19）：原 P1–P5 主体落地——**P1–P4 代码+测试完整合并**、**P5 自动化部分完成**；同期 2026-07-19 实机手动又派生 **P6–P9** 新立项。本节按状态分两块维护：「✅ 已完成」为最终快照（含日期+提交），「未来期望」按影响优先排列（手动验证尾巴 + 新立项）。
...
| P1 | Boss 狂暴 × 母舰对接冲突（位置 + 母舰停火） | 2026-07-19 | `f0821b7` | 实机发现副作用 → F5（原 P6） |
| P2 | 可变分辨率 16:9 + 大/中/小 三档 | 2026-07-19 | `ffa8675` | 代码+自动化测试完成；实机观感待 F1 |
| P3 | 固定攻击音效资产（bullet_fire） | 2026-07-19 | `90a6bec` | sfxr/Bfxr "Laser/Shoot" 配方 + 3 个 ±8% 音高变体；用户试听待 F2 |
| P4 | 排行榜服务器集成检测 | 2026-07-19 | `2e7f089` | 全链路 live E2E 通过；`.venv [server]` 缺失已修 |
| P5 | 次要功能回归检测（**自动化部分**） | 2026-07-19 | `ba14373` | 289 项测试全绿；5 项实机观感待 F3 |
| Rust 审计 | Rust 扩展逻辑审计 → 本地重编 | 2026-07-17 | wheel 0.1.0 → 0.2.0 | 210 项测试全绿 |

### 未来期望 / Future Backlog

按影响优先排列。F1–F4 是已落地代码的人工验证尾巴（含 P9 待复现确认），F5–F7 是 P6–P8 实机 bug 立项。

| # | 任务 | 来源 | 验收标准 | 关键模块 | 状态 |
|---|------|------|---------|---------|------|
| F1（原 P2 实机观感） | 三档分辨率 + 全屏 letterbox 实机观感 | P2 | ① 三档切换观感；② 全屏非 16:9 letterbox 黑边；③ 自由拖放（**已确认超出原验收①②范围**，可选放弃或升档） | `airwar/window/window.py`、`game/scaled_viewport.py` | 待实机 |
| F2（原 P3 试听） | bullet_fire 音效用户试听确认 | P3 | 实机试听 3 个音高变体轮转，方波 zap 音是否合预期；不满意改 `scripts/generate_bullet_fire_wav.py` 顶部参数重跑 | `airwar/assets/audio/bullet_fire*.wav` | 待确认 |
| F3（原 P5 实机手动） | P5 ①–③ ⑤ ⑥ 手动回归 | P5 | ① 冲刺观感/手感；② 返航基地+补给；③ 投降 Death 跳转；⑤ 全屏真实窗口往返+拖放观感（SDL dummy 无意义，需实机）；⑥ 对接中触发狂暴全流程 | 多模块 | 待实机 |
| F4（原 P9 复现确认） | 退出路径无 `Game flow ended` 日志 | P9 | 先确认当时退出方式（菜单 / 窗口 X / 异常消失）再决定是否立项；复现后再实施 | `game/scene_director.py`、`game/scene_director_components/scene_switcher.py` | 待确认 |
| F5（原 P6） | Boss 狂暴期间母舰钉住当前位置（而非断控+漂移） | P1⑦ 实机副作用 | ① 带初速度进入狂暴→位置逐帧不变；② 狂暴结束→输入恢复、从原位继续；③ 回归测试含残余速度清零 | `game/mother_ship/mother_ship_motion.py`、`game/mother_ship/game_integrator.py::_update_mothership_input` | 未开始 |
| F6（原 P7） | 母舰 H 提前脱离失效 + 冷却返还逻辑修正 | P5 实机副作用 | ① DOCKED 下 H 改电平触发；② 单一公式按比例返还（先写复现测试钉死"6%"来源）；③ 两条路径回归测试 | `game/mother_ship/input_detector.py`、`state_machine.py`、`game_integrator.py` | 未开始 |
| F7（原 P8） | 锁释放警告刷屏（热路径 token 化） | 实机日志噪声 | ① 热路径改 token 释放；② 未持锁跳过释放；③ warning 消除或降级 debug；④ 实机日志不再刷屏 | `game/managers/game_loop_manager.py`、`lock_manager.py` | 未开始 |
...
- **LockManager 优先级不变**：`MOTHERSHIP=80 > BOSS_ENRAGE=60` 是正确层级——玩家对接后理应保持无敌，不应改动。
- **P2 方案 A**：逻辑分辨率恒为 1920×1080，OS 窗口经 `pygame._sdl2.Window.size` 缩放；`ScaledViewport.present()` letterbox 是全屏处理路径（桌面 ≠ 16:9 时补黑边）。
- **P3 sfxr/Bfxr "Laser/Shoot" 配方**：方波载波 + 760→170 Hz 指数下滑扫频 + 2 ms attack / 指数衰减 + 5 ms 噪声瞬态；3 个 ±8% 音高变体轮转抗重复疲劳（变体预生成缓存，零运行时合成）。
- **Rust 重编命令**：`cd airwar_core && maturin build --release && pip install --force-reinstall target/wheels/airwar_core-*.whl`（无需 venv）；或 `maturin develop --release`（需 venv）。
