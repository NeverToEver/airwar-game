...
airwar/                 # 主 Python 包
  ...
  assets/               # 字体、音频、精灵图
  data/                 # 旧版包内数据目录（users.json 与生成缓存已迁往平台用户目录，仅保留迁移读取）
...
最近验证结果（2026-07-19）：`289 passed`，`ruff check .` 全绿，`compileall -q airwar main.py` 通过。注意：排行榜服务器测试（`test_leaderboard_server.py` / `test_leaderboard_live.py`）需要 `pip install -e ".[server]"`，否则被 `importorskip` 静默跳过。
...
|---|------|-----------|---------|---------|------|
| P1 | Boss 狂暴 × 母舰对接（位置冲突 + 母舰停火） | **子问题 A — 位置冲突**：更新流水线步骤 10（`_step_core_logic`）中，先执行 `update_game()` → `boss.update()` → `_center_player_for_enrage()` 将玩家 rect **直接移到屏幕中央**（绕过 LockManager，属于架构违规），随后 `if docked` 分支又将玩家 rect **覆盖回**对接舱坐标。两个位置写入在同一帧内先后发生，后者覆盖前者。**子问题 B — 母舰假装开火**：母舰实体的移动与开火由 `game_integrator.py` 独立控制，不经过 LockManager。狂暴期间 `game_integrator` 继续开火，但子弹对对接舱内玩家无效（MOTHERSHIP 锁保无敌），给玩家"在开火但造不成伤害"的假象。`MOTHERSHIP=80 > BOSS_ENRAGE=60` **是正确的优先级设计**——玩家对接后理应保持无敌，不应改动锁层 | **子问题 A**：① `_center_player_for_enrage` 改为通过 LockManager 请求玩家位置变更（或由 `_step_core_logic` 统一判定"对接中 → 跳过狂暴位移"）；② 任意时序下玩家坐标合法且确定；③ 新增自动化回归测试。**子问题 B**：④ 在 `game_integrator.py` 的 `update()` / `_update_mothership_firing()` / `_update_mothership_input()` 中检查 Boss 狂暴状态（`boss_state.is_enrage_active()` / `is_enrage_transitioning()`），狂暴期间跳过母舰移动输入和开火逻辑；⑤ 狂暴结束后自动恢复；⑥ 为 `game_integrator` 狂暴行为新增回归测试；⑦ 手动完整验证「对接中触发狂暴→母舰停火+锁定+玩家安全→狂暴结束恢复」全流程 | A：`entities/enemy/boss/boss.py::_center_player_for_enrage`、`scenes/game_scene_updater.py::_step_core_logic`、`game/managers/game_loop_manager.py`；B：`game/mother_ship/game_integrator.py`（`update()` / `_update_mothership_firing()` / `_update_mothership_input()`）、`entities/enemy/boss/boss_state.py` | 代码+回归测试完成 2026-07-19（`f0821b7`）：A 采用"_step_core_logic 统一判定"方案——Boss 新增 `player_position_locked` 帧级标志，对接时跳过狂暴位移并改抓玩家当前位置；B 在 `update()` 每帧缓存 `_boss_enrage_engaged`（Boss 新增公开谓词 `is_enrage_engaged()`），狂暴期间母舰输入清零+停火、在飞弹继续、结束后自动恢复。①–⑥ ✅；⑦ 实机验证已进行（2026-07-19）：对接中狂暴全流程走通、玩家安全且母舰停火，但发现母舰漂移+断控副作用 → 立项 P6 |
| P2 | 可变分辨率：固定长宽比 + 大/中/小三档 | 窗口默认 1920×1080 且可自由拖放（`Window._min_size`=1024×768 还是 4:3），VIDEORESIZE 任意改变宽高比，pygame `SCALED` 把 1920×1080 逻辑面直接拉伸到窗口，非 16:9 窗口下画面变形；设置界面无分辨率选项。**设计决策（2026-07-18）**：采用**方案 A**——逻辑分辨率保持 1920×1080 不变，只改变 OS 窗口尺寸，依赖 SDL2 SCALED 做 GPU 缩放。所有游戏坐标不变，鼠标坐标通过 `ScaledViewport.screen_to_logical()` 换算（16:9 窗口下天然无 letterbox，映射恒等）。**全屏注意事项**：`Window.toggle_fullscreen()` 当前使用 `pygame.FULLSCREEN`（不带 SCALED），若桌面非 16:9 会变形，须在 `ScaledViewport.present()` 走 letterbox 逻辑补黑边 | ① 大/中/小三档窗口尺寸全部锁定 16:9：**L**=2560×1440、**M**=1920×1080（默认）、**S**=1280×720；② `Window.resize()` 约束宽高比 16:9（`height = width * 9 // 16`），`_min_size` 改为 1280×720；③ `Window._get_adaptive_size()` 输出也约束 16:9；④ 设置界面新增分辨率下拉选项，切换后持久化到 `UserDB.settings["resolution_tier"]`（值为 `"S"` / `"M"` / `"L"`），启动时按上次档位恢复；⑤ 任意档位与全屏切换下画面不变形（全屏非 16:9 时 `ScaledViewport.present()` 补 letterbox 黑边）；⑥ 各档位下鼠标坐标映射正确（验证 `screen_to_logical` 换算） | `airwar/window/window.py`、`game/scaled_viewport.py`、`scenes/settings_scene.py`、`utils/database.py` | 代码+自动化测试完成 2026-07-19（`ffa8675`）：①②③ `Window.resize()` 锁定 16:9（`height=width*9//16`）、`_min_size`=1280×720、`_get_adaptive_size()` 输出约束 16:9；显示面恒定 1920×1080，OS 窗口经 `pygame._sdl2.Window.size` 缩放；④ 设置界面新增分辨率循环行，`resolution_tier` 持久化并即时生效，登录后 `_load_user_settings` 恢复；⑤ `ScaledViewport.present()` 新增 letterbox 路径（含 scratch surface 缓存，空间换时间）；⑥ `screen_to_logical` 按 OS 窗口尺寸自同步（缓存变换，变更才重算）。233 项测试全绿（含 venv 内 airwar_core 0.2.0 wheel 修复）。**实机验证待进行**：三档切换观感、全屏 letterbox、自由拖放（未锁定，仍可能变形——超出验收①②范围） |
| P3 | 固定攻击音效资产 | `bullet_fire` 目前由 numpy + `pygame.sndarray` 按各平台 mixer 采样率**程序生成**，每个平台音色都不同；`airwar/assets/audio/` 目录为空 | ① 生成 `bullet_fire.wav`（规格：**44100 Hz / 16-bit / mono**，内容与当前 `_generate_beep(frequency=880, harmonics=(1.0,0.35))` 一致），提交到 `airwar/assets/audio/`；② `_build_sfx("bullet_fire")` 改为优先加载 WAV 文件，文件缺失时回退到 numpy 程序生成（保留现有 `_generate_beep`）；③ 确认 PyInstaller 打包（`AirWar.spec`）包含 `airwar/assets/audio/`；④ 生成脚本放 `scripts/generate_bullet_fire_wav.py` | `airwar/audio/sound_manager.py`、`airwar/assets/audio/`、`AirWar.spec` | 完成 2026-07-19（`90a6bec`，音色由助手定为方波 zap 并经用户授权自决）：诊断——7.5 发/秒 × 固定音高纯音 = 疲劳源；采用业界 sfxr/Bfxr "Laser/Shoot" 配方（方波载波 + 760→170 Hz 指数下滑扫频 + 2ms attack/指数衰减 + 5ms 噪声瞬态），生成 3 个 ±8% 音高变体轮转播放抗重复疲劳（空间换时间：变体预生成缓存，零运行时合成）。①②③④ ✅。9 项新测试全绿。实机试听待用户确认；不满意改 `scripts/generate_bullet_fire_wav.py` 顶部参数重跑即可 |
| P4 | 排行榜服务器集成检测 | FastAPI 远程排行榜（`run_with_server.py`）从未实测，仅单元测试覆盖。测试范围：**本地 FastAPI 模式**（`AIRWAR_LEADERBOARD_MODE=local`），无需远程服务器 | ① 服务器启动 → 游戏内成绩提交 → 排行榜拉取 → 数据显示全链路手动验证通过；② 对照 `.env.example` 确认所有排行榜相关环境变量均被 `service.py` 正确消费，缺字段/字段名不一致的修正 `.env.example`；③ 发现的问题单独立项 | `airwar/leaderboard/`、`run_with_server.py`、`.env.example` | 完成 2026-07-19（`2e7f089`）：① ✅ 新增 `tests/test_leaderboard_live.py` 8 项实测回归——真实 uvicorn 服务器（线程内 + CLI 入口 `python -m airwar.leaderboard.server` 两条启动路径）× 生产 `RemoteLeaderboardClient`（urllib）× `LeaderboardService` 三模式（remote/auto/local）+ auto 模式服务器宕机回退本地 + `LeaderboardView` 拉取与页脚 i18n；`run_with_server.py` 无头冒烟通过（依赖检测→健康等待→游戏启动→干净关停）；显示层截图确认（CJK 名字、名次排序、页脚「Top 10 · 全球排行榜」均正确）。② ✅ `.env.example` 5 个变量（URL/MODE/TIMEOUT/DB_PATH/CORS_ORIGINS）均被 `config.py` 正确消费，名称一致，无需修正。③ 唯一发现：`.venv` 缺 `[server]` 依赖导致 `test_leaderboard_server.py` 被 importorskip 整文件静默跳过——已 `pip install -e ".[server]"` 修复（11 项服务器单测恢复执行）；链路代码本身无缺陷，无需立项。264 项测试全绿 |
| P5 | 次要功能回归检测（持续追加） | 部分边缘功能缺乏近期实机验证。**本任务为持续追加项**：P1/P2 修复完成后须追加对应回归条目 | 手动过一遍：① 相位冲刺（按-松 Shift → 瞬移 + 无敌帧）；② 返航 homecoming（长按 B 2.4s → 回基地 → 补给）；③ 投降 give-up（长按 K 3s → 放弃出击）；④ 设置项持久化（改设置 → 退游戏 → 重进验证）；⑤ 窗口缩放/全屏切换（拖放 + 全屏往返）；⑥ **P1 修复后追加**：对接中触发狂暴全流程；⑦ **P2 修复后追加**：三档分辨率切换 + 鼠标映射。发现问题单独立项 | 多模块 | 自动化部分完成 2026-07-19（`ba14373`）：①②③ 新增 `tests/test_gameplay_flows.py` 17 项——冲刺全链（Shift 轻按→位移 250px/25% 能量消耗/无敌帧/90 帧冷却 + 天赋未解锁/能量不足/控制锁三条阻断路径）、返航协调器全链（长按 B 检测→HOMECOMING 锁（无敌+暂停+控锁）→返航序列状态机相位推进→基地补给 RP 扣减→离站解锁+重生位重置）、投降全链（长按 K 检测→暂停/奖励选择器门控→INSTANT_KILL 标准击杀路径）；④⑦ 新增 `tests/test_settings_persistence.py` 6 项——UserDB settings API 跨实例往返+默认值合并独立性、设置界面档位落盘、登录后 `_load_user_settings` 恢复并即时应用分辨率档位（⑦ 共享路径）、Guest 跳过；⑥ `test_boss.py` +1（真实 Boss `is_enrage_engaged` 谓词）、`test_lock_manager.py` +1（MOTHERSHIP=80 > BOSS_ENRAGE=60 对接玩家无敌保持）。289 项测试全绿。**仍需实机手动**：①-③ 的观感/手感（状态转换正确性已由自动化覆盖）、⑤ 全屏真实窗口往返与拖放观感（SDL dummy 驱动下无意义）、⑥ 对接中触发狂暴实机全流程、③ 投降后 Death 场景跳转的实机确认 |
| P6 | Boss 狂暴期间母舰钉住当前位置（而非断控+漂移） | 实机反馈（2026-07-19，P1⑦ 验证的副作用）：狂暴期间母舰**带着残余速度漂移**——P1-B 只把 `set_player_input(0,0)` 清零输入，未清零速度；`MotherShipMotion.FRICTION=1.0`（无摩擦设计，`mother_ship_motion.py:44`）使残余速度持续滑行直至撞场地边界。同时玩家感到控制被直接切断。设计意图：狂暴时母舰**钉在当前位置**（速度清零+跳过运动积分），狂暴结束后从原位恢复操控 | ① 带初速度进入狂暴 → 母舰位置逐帧不变；② 狂暴结束 → 输入恢复、从原位继续；③ 新增回归测试（含残余速度清零断言） | `game/mother_ship/mother_ship_motion.py`（新增位置锁/冻结机制）、`game/mother_ship/game_integrator.py::_update_mothership_input` | 未开始 |
| P7 | 母舰 H 提前脱离失效 + 冷却返还逻辑修正 | **子问题 1 — 提前脱离失效**（实机反馈 2026-07-19）：`input_detector.py:37` 边沿触发 `EVENT_H_PRESSED`，从对接长按一路按住 H 进入 DOCKED 不产生新边沿 → `_exit_in_progress` 永不置位 → 退出蓄力永不启动；唯一可用脱离方式是松手重按 2s，"提前摁住快速脱离"路径是死的。**子问题 2 — 返还逻辑错误**：补偿分裂为两套互不知情的叠加机制——`_calculate_undocking_cooldown_multiplier`（`max(0.6, 1.0-剩余×0.4)`，`game_integrator.py:498`，撤离动画开始时按剩余驻留算冷却倍率）+ `_exit_refund_progress`（`min(0.3, 剩余×0.5)`，`game_integrator.py:462`，手动退出再叠加冷却进度头开始），早退被双重补偿；实机观察到"固定 6%"（≈88% 驻留手动退出的 `0.12×0.5`）。设计规则：**耗尽弹药撤离 = 零补偿**；手动早退 = 单一按比例返还。**待拍板**：a) 狂暴期间是否允许 H 快速脱离（建议允许，脱离前保持无敌）；b) 提前按住 H 进入 DOCKED 后立即脱离还是从 DOCKED 起计 2s；c) 返还公式保留"冷却进度头开始"还是"冷却倍率" | ① DOCKED 下 H 改电平触发：当前按住且未在退出流程即启动退出蓄力，持续按住时间计入 2.0s 要求；从对接持续按住 H → 按拍板规则快速脱离；松手取消；② 补偿改为按退出原因的单点计算：耗尽撤离零补偿（先写复现测试钉死"6%"确切来源）、手动早退单一公式按比例返还；③ 两条路径回归测试 | `game/mother_ship/input_detector.py`、`game/mother_ship/state_machine.py`、`game/mother_ship/game_integrator.py`（`_on_exit_complete` / `_calculate_undocking_cooldown_multiplier` / `_on_cooldown_started`） | 未开始 |
| P8 | 锁释放警告刷屏（热路径 token 化） | 实机日志（2026-07-19）：狂暴未激活期间 `game_loop_manager.py:274` 每帧 `release(BOSS_ENRAGE)`，单秒 17 条 "Releasing lock by layer without token" 警告；`PLAYER_HIT` / `MOTHERSHIP` 释放点同样未用 token，淹没真实日志 | ① 热路径改为 token 释放；② 未持锁时跳过释放；③ 该 warning 消除或降级 debug；④ 实机日志不再刷屏 | `game/managers/game_loop_manager.py`、`game/systems/lock_manager.py`、各 release 调用点 | 未开始 |
| P9 | 退出路径无 `Game flow ended` 日志（待确认） | 实机日志（2026-07-19）：14:07:52 死亡后从对接存档恢复并删除存档，随后进程直接结束，无 07-17 那样的 `Game flow ended: ...` 记录。需先确认当时退出方式（菜单正常退出 / 窗口 X / 窗口异常消失）再决定是否立项 | 待确认复现方式后定 | `game/scene_director.py`、`game/scene_director_components/scene_switcher.py` | 待确认 |
...
- **P6–P9 来源**：2026-07-19 实机手动测试（约 90 秒对局顺带验证通过：P1⑦ 对接中狂暴玩家安全+母舰停火、P5③ 投降 INSTANT_KILL→Death→对接存档恢复链正常、会话零错误零崩溃）。P6 是 P1-B 修复的副作用（输入清零未清零速度）；P7 两个子问题同在母舰对接子系统，建议合并修复一次实机验证；P8 为日志噪声；P9 待确认退出方式。
