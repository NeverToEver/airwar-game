...
一款基于 Python + Pygame 的 2D 空战射击游戏，支持可选 Rust 原生扩展加速。当前版本重点强化了 Boss 战、基地指挥中心、鼠标辅瞄手感，以及运行期生成素材的本地缓存。
...
- 基地指挥中心：长按 B 返航后进入停机坪基地，支持武器路线切换（散射/激光）、天赋路线切换（相位冲刺/母舰冷却减半）、维修补给与配置保存，再次按 B 出发并触发轨道打击清屏。
