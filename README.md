...
```
1. 安装 Python（如果没有） → https://www.python.org/downloads/
...
**Windows:**
```powershell
# 1. 打开 PowerShell（按 Win+X，选择"终端"或"PowerShell"）

# 2. 检查 Python 是否已安装
python --version

# 3. 如果没有安装，下载安装：https://www.python.org/downloads/
...
|------|------|
| **三种难度** | 简单 / 普通 / 困难 |
| **Boss 战** | 每30秒出现 Boss，击败获得高额分数 |
| **Roguelike 增益** | 达成里程碑可选择强化效果 |
| **视觉风格** | 赛博朋克霓虹灯风格 |
...
| 问题 | 解决方案 |
|------|----------|
| "python 不是内部或外部命令" | 需要安装 Python，或将 Python 添加到系统 PATH |
| "pip 不是内部或外部命令" | 运行 `python -m pip install pygame` |
| 游戏窗口无法打开 | 检查显卡驱动是否更新 |
| 感觉太难/太简单 | 在主菜单按 W/S 切换难度 |
