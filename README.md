...
- 528+ 测试用例，100% 通过率，SOLID 架构设计
- 死亡动画系统（闪烁，火花，光晕）
- 母舰系统（游戏暂停、清敌、保存进度）
- 17 种 Roguelike 增益效果
- Surface 缓存优化，高性能渲染
- 实时难度系数显示面板（Sci-fi 霓虹风格）
...
│   │   └── systems/          # 游戏系统 (含 DifficultyCoefficientPanel)
├── scenes/               # 场景管理
│   ├── ui/              # 场景 UI 组件 (P5 新增)
│   │   ├── background.py # 背景渲染器
│   │   ├── particles.py  # 粒子系统
...
> **【已实现】非线性难度增长系统**
> 
> 基于 **Boss 击杀数量** 实现指数级难度增长，并区分简单/普通/困难三种初始模式。
> 
> 📄 详细设计文档：`docs/designs/nonlinear_difficulty_system_design.md`
...
> **Explosive 天赋**: 需击杀至少 2 个 Boss 后才可选择
...
**新增组件** (`airwar/game/systems/difficulty_coefficient_panel.py`):

| 组件 | 职责 | 特性 |
|------|------|------|
| DifficultyCoefficientPanel | 实时显示难度系数变化 | Sci-fi 霓虹发光效果 |

**功能调整**:

| 变更 | 说明 |
|------|------|
| Explosive 天赋解锁条件 | 从 `cycle_count > 2` 改为 `boss_kill_count >= 2` |
| DifficultyManager API | 新增 `initial_multiplier` 属性 |

**Bug 修复**:

- 修复 `on_boss_killed` 回调未传递 `score` 参数的问题
