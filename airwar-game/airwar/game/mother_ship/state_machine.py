...
class MotherShipStateMachine(IMotherShipStateMachine):
    ...
    def transition(self, event: str, **kwargs) -> None:
        handlers = {
            'H_PRESSED': self._on_h_pressed,
            'H_RELEASED': self._on_h_released,
            'PROGRESS_COMPLETE': self._on_progress_complete,
            'DOCKING_ANIMATION_COMPLETE': self._on_docking_animation_complete,
            'UNDOCKING_ANIMATION_COMPLETE': self._on_undocking_animation_complete,
        }
    ...
    def _on_h_released(self, **kwargs) -> None:
        if self._current_state == MotherShipState.PRESSING and self._can_transition_to(MotherShipState.IDLE):
