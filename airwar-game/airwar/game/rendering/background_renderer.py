...
class BackgroundRenderer:
    ...
    def _generate_gradient(self) -> None:
        self._cached_gradient = SurfaceCache.get_vertical_gradient(
            self.screen_width,
            self.screen_height,
            (5, 5, 20),
            (15, 13, 50),
    ...
    def draw(self, surface: pygame.Surface) -> None:
        ...
        for particle in self.particles:
            ...
            particle_surf = SurfaceCache.get_particle_glow(size, color, alpha)
            surface.blit(particle_surf, (int(particle['x'] - size * 2), int(particle['y'] - size * 2)))
