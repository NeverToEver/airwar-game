...
class HUDRenderer:
    def __init__(self):
        ...
        self.boss_font = pygame.font.Font(None, 24)
    ...
    def render_boss_health_bar(self, surface: pygame.Surface, boss) -> None:
        ...
        boss_text = self.boss_font.render("BOSS", True, (255, 255, 255))
        ...
        time_text = self.boss_font.render(f"{time_remaining:.1f}s", True, (255, 220, 100))
        time_rect = time_text.get_rect(right=x + bar_width - 8, centery=y + bar_height // 2)
        surface.blit(time_text, time_rect)

        progress = boss.get_survival_progress()
        if progress > 0.7:
            warning_text = self.boss_font.render("HURRY!", True, (255, 100, 100))
