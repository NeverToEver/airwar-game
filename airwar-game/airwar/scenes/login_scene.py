...
class LoginScene(Scene):
    ...
    def _draw_gradient_background(self, surface: pygame.Surface) -> None:
        ...
        surface.blit(
            SurfaceCache.get_vertical_gradient(width, height, self.colors['bg'], self.colors['bg_gradient'], step=3),
            (0, 0),
        )
    ...
    def _render_panel(self, surface, width, height):
        ...
        for i in range(4, 0, -1):
            ...
            glow_surf = SurfaceCache.get_rect_glow(
                self.panel_width + expand * 2,
                self.panel_height + expand * 2,
                (*self.colors['title_glow'], alpha),
                border_radius=18,
            )
            surface.blit(glow_surf, (self.panel_x - expand, self.panel_y - expand))

        surface.blit(
            SurfaceCache.get_rect_fill(self.panel_width, self.panel_height, self.colors['panel'], border_radius=15),
            panel_rect.topleft,
        )
        border_surf = SurfaceCache.get_rect_border(
            self.panel_width,
            self.panel_height,
            (*self.colors['panel_border'], 150),
            border_width=2,
            border_radius=15,
        )
    ...
    def _draw_input_bg(self, surface, rect, is_active):
        ...
        surface.blit(SurfaceCache.get_rect_fill(rect.width, rect.height, bg_color, border_radius=10), rect.topleft)
        border_color = self.colors['title'] if is_active else self.colors['panel_border']
        border_surf = SurfaceCache.get_rect_border(
            rect.width,
            rect.height,
            (*border_color, 180 if is_active else 120),
            border_width=2,
            border_radius=10,
        )
    ...
    def _render_button(self, surface, rect, text, color, is_primary):
        ...
        surface.blit(SurfaceCache.get_rect_fill(rect.width, rect.height, btn_color, border_radius=10), rect.topleft)
        border_surf = SurfaceCache.get_rect_border(
            rect.width,
            rect.height,
            (*self.colors['title_glow'], 120 if hover else 100),
            border_width=2,
            border_radius=10,
        )
        surface.blit(border_surf, rect.topleft)
