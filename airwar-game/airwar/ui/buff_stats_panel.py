...
class BuffStatsPanel:
    ...
    def _create_panel_surface(self, width: int, height: int) -> pygame.Surface:
        ...
        surface.blit(
            SurfaceCache.get_rect_fill(width, height, self._bg_color, border_radius=self._corner_radius),
            (0, 0),
        )
        surface.blit(
    ...
    def _build_cache_signature(
        ...
        entry_signature = tuple(
            (entry.short_name, entry.value, entry.level, entry.color, entry.category)
            for entry in buff_entries
        )
        summary_signature = tuple(summary.items())
        return entry_signature, summary_signature
    ...
    def render(
        ...
        try:
            ...
            cache_signature = self._build_cache_signature(buff_entries, summary)
            if (
                self._cached_surface is None
                or self._cached_signature != cache_signature
                or self._cached_surface.get_height() != panel_height
            ):
                ...
                self._cached_surface = panel_surface
                self._cached_signature = cache_signature

            surface.blit(self._cached_surface, (panel_x, panel_y))
