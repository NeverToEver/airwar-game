...
class RewardSelector:
    ...
    def _draw_panel(self, surface: pygame.Surface) -> None:
        ...
        for i in range(3, 0, -1):
            ...
            glow_surf = SurfaceCache.get_rect_glow(
                panel_width + expand * 2,
                panel_height + expand * 2,
        ...
        surface.blit(
            SurfaceCache.get_rect_fill(panel_width, panel_height, self.colors['panel'], border_radius=15),
            panel_rect.topleft,
        )
        border_surf = SurfaceCache.get_rect_border(
            panel_width,
            panel_height,
            (*self.colors['panel_border'], 120),
            border_width=2,
            border_radius=15,
        )
    ...
    def _draw_option_item(self, surface: pygame.Surface, option: dict, index: int,
        ...
        if is_selected:
            ...
            surface.blit(
                SurfaceCache.get_rect_fill(box_width, box_height, self.colors['option_selected_bg'], border_radius=10),
                box_rect.topleft,
            )
            border_surf = SurfaceCache.get_rect_border(
                box_width,
                box_height,
                (*self.colors['selected'], 200),
                border_width=2,
                border_radius=10,
            )
            ...
            surface.blit(
                SurfaceCache.get_rect_fill(box_width, box_height, self.colors['option_unselected_bg'], border_radius=10),
                box_rect.topleft,
            )
            border_surf = SurfaceCache.get_rect_border(
                box_width,
                box_height,
                (*self.colors['unselected'], 70),
                border_width=1,
                border_radius=10,
            )
            surface.blit(border_surf, box_rect.topleft)
