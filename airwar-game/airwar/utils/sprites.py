...
_SPRITE_SURFACE_CACHE: dict = {}
_MAX_SPRITE_CACHE_ENTRIES = 128


def _get_health_stage(health_ratio: float) -> str:
    if health_ratio > 0.6:
        return 'high'
    if health_ratio > 0.3:
        return 'mid'
...
def _get_cached_sprite(cache_key: tuple, builder):
    cached = _SPRITE_SURFACE_CACHE.get(cache_key)
    if cached is not None:
        _SPRITE_SURFACE_CACHE.pop(cache_key)
        _SPRITE_SURFACE_CACHE[cache_key] = cached
        return cached

    cached = builder()
    _SPRITE_SURFACE_CACHE[cache_key] = cached
    if len(_SPRITE_SURFACE_CACHE) > _MAX_SPRITE_CACHE_ENTRIES:
        oldest_key = next(iter(_SPRITE_SURFACE_CACHE))
        _SPRITE_SURFACE_CACHE.pop(oldest_key, None)
    return cached


def clear_sprite_cache() -> None:
    _SPRITE_SURFACE_CACHE.clear()


def _blit_cached_sprite(surface: pygame.Surface, sprite_data, x: float, y: float) -> None:
    sprite_surface, pad_x, pad_y = sprite_data
    surface.blit(sprite_surface, (int(x - pad_x), int(y - pad_y)))
...
def draw_player_ship(surface: pygame.Surface, x: float, y: float, width: float = 50, height: float = 60) -> None:
    ...
    cache_key = ('player', width_int, height_int)

    def _build():
        pad = max(width_int, height_int)
        sprite_surface = pygame.Surface((width_int + pad * 2, height_int + pad * 2), pygame.SRCALPHA)
        _render_player_ship(sprite_surface, pad, pad, width_int, height_int)
        return sprite_surface, pad, pad

    _blit_cached_sprite(surface, _get_cached_sprite(cache_key, _build), x, y)


def _render_player_ship(surface: pygame.Surface, x: float, y: float, width: float = 50, height: float = 60) -> None:
    center_x = x + width / 2
    hull_dark = (35, 60, 100)
    hull_mid = (55, 100, 160)
...
def draw_enemy_ship(surface: pygame.Surface, x: float, y: float, width: float = 40, height: float = 40, health_ratio: float = 1.0) -> None:
    width_int = max(1, int(round(width)))
    height_int = max(1, int(round(height)))
    cache_key = ('enemy', width_int, height_int, _get_health_stage(health_ratio))

    def _build():
        pad = max(width_int, height_int)
        sprite_surface = pygame.Surface((width_int + pad * 2, height_int + pad * 2), pygame.SRCALPHA)
        _render_enemy_ship(sprite_surface, pad, pad, width_int, height_int, health_ratio)
        return sprite_surface, pad, pad

    _blit_cached_sprite(surface, _get_cached_sprite(cache_key, _build), x, y)


def _render_enemy_ship(surface: pygame.Surface, x: float, y: float, width: float = 40, height: float = 40, health_ratio: float = 1.0) -> None:
    center_x = x + width / 2
...
def draw_single_bullet(surface: pygame.Surface, x: float, y: float, width: float, height: float) -> None:
    width_int = max(1, int(round(width)))
    height_int = max(1, int(round(height)))
    cache_key = ('bullet_single', width_int, height_int)

    def _build():
        pad = max(12, max(width_int, height_int))
        sprite_surface = pygame.Surface((width_int + pad * 2, height_int + pad * 2), pygame.SRCALPHA)
        _render_single_bullet(sprite_surface, pad, pad, width_int, height_int)
        return sprite_surface, pad, pad

    _blit_cached_sprite(surface, _get_cached_sprite(cache_key, _build), x, y)


def _render_single_bullet(surface: pygame.Surface, x: float, y: float, width: float, height: float) -> None:
    center_x = x + width / 2
    top_y = y
...
def draw_spread_bullet(surface: pygame.Surface, x: float, y: float, width: float, height: float) -> None:
    width_int = max(1, int(round(width)))
    height_int = max(1, int(round(height)))
    cache_key = ('bullet_spread', width_int, height_int)

    def _build():
        pad = max(12, max(width_int, height_int))
        sprite_surface = pygame.Surface((width_int + pad * 2, height_int + pad * 2), pygame.SRCALPHA)
        _render_spread_bullet(sprite_surface, pad, pad, width_int, height_int)
        return sprite_surface, pad, pad

    _blit_cached_sprite(surface, _get_cached_sprite(cache_key, _build), x, y)


def _render_spread_bullet(surface: pygame.Surface, x: float, y: float, width: float, height: float) -> None:
...
def draw_laser_bullet(surface: pygame.Surface, x: float, y: float, width: float, height: float) -> None:
    width_int = max(1, int(round(width)))
    height_int = max(1, int(round(height)))
    cache_key = ('bullet_laser', width_int, height_int)

    def _build():
        pad = max(12, max(width_int, height_int))
        sprite_surface = pygame.Surface((width_int + pad * 2, height_int + pad * 2), pygame.SRCALPHA)
        _render_laser_bullet(sprite_surface, pad, pad, width_int, height_int)
        return sprite_surface, pad, pad

    _blit_cached_sprite(surface, _get_cached_sprite(cache_key, _build), x, y)


def _render_laser_bullet(surface: pygame.Surface, x: float, y: float, width: float, height: float) -> None:
    center_x = x + width / 2
...
def draw_boss_ship(surface: pygame.Surface, x: float, y: float, width: float = 120, height: float = 100, health_ratio: float = 1.0) -> None:
    width_int = max(1, int(round(width)))
    height_int = max(1, int(round(height)))
    cache_key = ('boss', width_int, height_int, _get_health_stage(health_ratio))

    def _build():
        pad = max(width_int, height_int)
        sprite_surface = pygame.Surface((width_int + pad * 2, height_int + pad * 2), pygame.SRCALPHA)
        _render_boss_ship(sprite_surface, pad, pad, width_int, height_int, health_ratio)
        return sprite_surface, pad, pad

    _blit_cached_sprite(surface, _get_cached_sprite(cache_key, _build), x, y)


def _render_boss_ship(surface: pygame.Surface, x: float, y: float, width: float = 120, height: float = 100, health_ratio: float = 1.0) -> None:
    center_x = x + width / 2
    center_y = y + height / 2
