...
class Game:
    ...
    def run(self) -> None:
        while self.running:
            ...
            while self.running:
                ...
                for event in events:
                    if escape_handled and event.type == pygame.KEYDOWN and event.key == pygame.K_ESCAPE:
                        continue
                    self.scene_manager.handle_events(event)
    ...
    def _show_pause_menu(self) -> None:
        ...
        while pause_scene.is_paused() if hasattr(pause_scene, 'is_paused') else pause_scene.running:
            ...
            pause_scene.update()
            pause_scene.render(self.screen)
            pygame.display.flip()
            self.clock.tick(60)
    ...
    def _show_game_over_screen(self, score: int, kills: int) -> None:
        ...
        while waiting:
            ...
            self.screen.blit(title, title.get_rect(center=(self.screen_width // 2, int(150 * scale))))

            score_text = font_medium.render(f"SCORE: {score}", True, (255, 255, 255))
            self.screen.blit(score_text, score_text.get_rect(center=(self.screen_width // 2, int(280 * scale))))

            kills_text = font_medium.render(f"KILLS: {kills}", True, (200, 200, 100))
            self.screen.blit(kills_text, kills_text.get_rect(center=(self.screen_width // 2, int(330 * scale))))

            if self.current_user:
                user_data = self.db.get_user_data(self.current_user)
                high_score = user_data.get('high_score', 0)
                hs_text = font_small.render(f"HIGH SCORE: {high_score}", True, (100, 255, 150))
                self.screen.blit(hs_text, hs_text.get_rect(center=(self.screen_width // 2, int(400 * scale))))

            hint = font_small.render("ENTER to menu | ESC to quit", True, (100, 100, 150))
            self.screen.blit(hint, hint.get_rect(center=(self.screen_width // 2, int(500 * scale))))
