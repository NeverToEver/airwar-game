...
class StarLayer:
    ...
    def render(self, surface: pygame.Surface, time: float) -> None:
        for star in self._stars:
            ...
            twinkle_phase = (time * star["twinkle_speed"] + star["twinkle_offset"]) * (self._sin_table_size / math.tau)
            ...
            if brightness > self.GLOW_BRIGHTNESS_THRESHOLD:
                ...
                if cache_key not in self._glow_cache:
                    ...
                    pygame.draw.circle(glow_surf, (*self._color, glow_alpha), (glow_radius, glow_radius), glow_radius)
                    self._glow_cache[cache_key] = glow_surf
                surface.blit(
                    self._glow_cache[cache_key],
                    (x - glow_radius, y_pos - glow_radius),
...
class DustLayer:
    ...
    def __init__(self, screen_width: int, screen_height: int, count: int, speed_range: tuple[float, float]):
        self._screen_width = screen_width
        self._screen_height = screen_height
        ...
        for d in self._dust:
            ...
            if cache_key not in self._glow_cache:
                ...
                for layer in range(size * 2, 0, -2):
                    ...
                    pygame.draw.circle(dust_surf, (*particle_color, layer_alpha), (size * 2, size * 2), layer)
                self._glow_cache[cache_key] = dust_surf
            surface.blit(self._glow_cache[cache_key], (x - size * 2, y - size * 2), special_flags=pygame.BLEND_RGBA_ADD)
