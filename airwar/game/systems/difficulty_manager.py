...
class DifficultyManager:
    ...
    def on_boss_killed(self) -> None:
        ...
        self._logger.info(
            f"Boss #{self._boss_kill_count} killed - multiplier: {old_multiplier:.2f} -> {self._current_multiplier:.2f}"
        )
    ...
    def _update_multiplier(self) -> None:
        ...
        self._current_multiplier = min(raw_multiplier, self._strategy.max_multiplier)
        self._cache_dirty = True
    ...
    def _calculate_params(self) -> dict[str, Any]:
        ...
        params = {
            ...
            "spawn_rate": max(
                self.MIN_SPAWN_RATE, int(BASE_ENEMY_PARAMS["spawn_rate"] / (1 + speed_mult * self.SPAWN_SCALE))
            ),
