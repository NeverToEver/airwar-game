...
class WelcomeScene(Scene, MouseInteractiveMixin):
    ...
    def handle_events(self, event: pygame.event.Event) -> None:
        if event.type == pygame.KEYDOWN:
            ...
            if self.handle_mouse_click(event.pos):
                ...
                # Clicking on input areas sets focus
    ...
    def _render_left_panel(self, surface, px, py):
        ...
        # Skip login link (guest mode)
        skip_y = btn_y + self.BTN_H + 18
        skip_font = pygame.font.Font(None, self._tokens.typography.SMALL_SIZE)
        skip_text = skip_font.render("Play as Guest", True, SC.TEXT_DIM)
        skip_rect = skip_text.get_rect(center=(px + self.PANEL_W // 2, skip_y))
        surface.blit(skip_text, skip_rect)
        self.register_button('skip_login',
                            pygame.Rect(skip_rect.x - 10, skip_rect.y - 5,
                                       skip_rect.width + 20, skip_rect.height + 10))
    ...
    def _draw_button(self, surface, rect, text, btn_name, color, is_primary=False):
        ...
        text_color = SC.TEXT_BRIGHT if hover else SC.TEXT_PRIMARY
