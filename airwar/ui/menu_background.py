...
class MenuBackground:
    ...
    def _get_light_surface(self, width: float, height: int) -> pygame.Surface:
        ...
        if cache_key not in MenuBackground._light_surface_cache:
            ...
            points = [(width * 0.3, 0), (width * 1.7, 0), (width * 2, height), (0, height)]
    ...
    def _render_scan_beams(self, surface: pygame.Surface, width: int, height: int) -> None:
        ...
        for beam in self._scan_beams:
            ...
            if beam.orientation == "h":
                glow_surf = self._get_scan_glow_surface("h", width, glow_radius, alpha)
                y_pos = int(beam.position) - glow_radius - 2
                surface.blit(glow_surf, (0, y_pos))
            else:
                glow_surf = self._get_scan_glow_surface("v", height, glow_radius, alpha)
