...
class PauseButtonComponent:
    ...
    def _ensure_cache(self, colors, spacing) -> None:
        ...
        for state_key, bg_alpha, border_alpha in [
            ...
            pygame.draw.rect(bg_surface, bg_color, bg_surface.get_rect(), border_radius=spacing.BORDER_RADIUS_SM)

            border_color = (*colors.PANEL_BORDER, border_alpha)
            border_surface = pygame.Surface((size, size), pygame.SRCALPHA)
            pygame.draw.rect(
                border_surface, border_color, border_surface.get_rect(), width=1, border_radius=spacing.BORDER_RADIUS_SM
            )
