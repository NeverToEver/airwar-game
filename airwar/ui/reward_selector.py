...
class RewardSelector(MouseSelectableMixin):
    ...
    def show(
        self, options: list, callback: Callable, buff_levels: dict | None = None, unlocked_buffs: list | None = None
    ) -> None:
        self.visible = True
    ...
    def _draw_panel(self, surface: pygame.Surface) -> None:
        ...
        for i in range(3, 0, -1):
            ...
            alpha = max(5, 20 // i)
            pygame.draw.rect(glow_surf, (*self.colors["title_glow"], alpha), glow_surf.get_rect(), border_radius=18)
        ...
        pygame.draw.rect(
            border_surf, (*self.colors["panel_border"], 120), border_surf.get_rect(), width=2, border_radius=15
        )
    ...
    def render(self, surface: pygame.Surface) -> None:
        ...
        for i, option in enumerate(self.options):
            if self.use_themed_style:
                self._draw_themed_option_item(surface, option, i, center_x, start_y, i == effective_index, box_width)
            else:
                self._draw_option_item(surface, option, i, center_x, start_y, i == effective_index, box_width)
    ...
    def _draw_themed_option_item(
        ...
        draw_chamfered_panel(surface, box_rect.x, box_rect.y, box_width, box_height, bg_color, border_color, None, 8)
